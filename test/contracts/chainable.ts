...
import ProcessBuilder, { DEFAULT_ETH_CHAIN_ID, DEFAULT_PARAMS_SIGNATURE } from "../builders/process"
...
describe("Chainable Process contract", () => {
    ...
    it("should fail deploying if the predecessor address is not a contract", async () => {
        ...
        try {
            await contractFactory.deploy(Wallet.createRandom().address, namespaceInstance.address, resultsIntance.address, storageProofAddress, ethChainId) as Contract & ProcessContractMethods
    ...
    describe("Instance forking", () => {
        it("should allow to deploy a contract with no predecessorAddress", async () => {
            ...
            const contractFactory = new ContractFactory(processAbi, processByteCode, entityAccount.wallet)
            const localInstance1: Contract & ProcessContractMethods = await contractFactory.deploy(nullAddress, namespaceInstance1.address, resultsIntance.address, storageProofAddress, ethChainId) as Contract & ProcessContractMethods
        ...
        it("should not allow to deploy with itself as a predecessor", async () => {
            ...
            try {
                await contractFactory.deploy(nextContractDeployAddress, namespaceInstance1.address, resultsIntance.address, storageProofAddress, ethChainId) as Contract & ProcessContractMethods
        ...
        it("should retrieve the predecessorAddress if set", async () => {
            ...
            const namespaceInstance1 = await new NamespaceBuilder().build()
            const contractFactory = new ContractFactory(processAbi, processByteCode, entityAccount.wallet)
            contractInstance = await contractFactory.deploy(predecessorAddress, namespaceInstance1.address, resultsIntance.address, storageProofAddress, ethChainId) as Contract & ProcessContractMethods
